...
uint8_t grid_rows; // No initial value
uint8_t grid_cols; // No initial value

#define MAX_MAP_CELLS_COUNT 100 // Set a reasonable maximum

Cell_map_t grid_map_cells[MAX_MAP_CELLS_COUNT] = { 0 };
Cell_map_t *ptr_current_cell;
Cell_map_t *ptr_goal_cell;
Cell_map_t *ptr_start_cell;

Cell_map_t *open_list[MAX_MAP_CELLS_COUNT];
uint8_t open_list_index = 0;

Cell_map_t *closed_list[MAX_MAP_CELLS_COUNT];
uint8_t closed_list_index = 0;

char *map_string[MAX_MAP_CELLS_COUNT];

/* Prototipos de las funciones del main */
void init_empty_grid_map(uint8_t gridCols, uint8_t gridRows,
		Cell_map_t *cellArray);
...
Cell_map_t* get_cell_start(Cell_map_t *grid_map, uint8_t gridCols,
		uint8_t gridRows);
Cell_map_t* get_cell_goal(Cell_map_t *grid_map, uint8_t gridCols,
		uint8_t gridRows);
...
uint16_t get_H_cost(Cell_map_t *goal_cell, Cell_map_t *working_cell) {
	...
	float32_t distance;
	arm_sqrt_f32((float32_t) (dx * dx + dy * dy), &distance);
...
uint16_t get_G_cost(Cell_map_t *neighbour_cell, Cell_map_t *working_cell) {
	...
	float32_t distance_cost;
	arm_sqrt_f32((float32_t) (dx * dx + dy * dy), &distance_cost);
	uint16_t aux_result = (uint16_t) distance_cost + working_cell->g;
...
void init_empty_grid_map(uint8_t gridCols, uint8_t gridRows,
	...
	for (int i = 0; i < gridRows; i++) {
		for (int j = 0; j < gridCols; j++) {
			cellArray[i * gridCols + j] = create_cell(j * GRID_SIZE,
					i * GRID_SIZE);
...
bool expecting_map_data = false;
uint8_t rows_received = 0;

void parseCommands(char *ptrBufferReception) {
    char cmd[MAX_CMD_LENGTH];
    int numCols, numRows;

    // Parse the command and dimensions
    sscanf(ptrBufferReception, "%s %d %d", cmd, &numCols, &numRows);

    if (strcmp(cmd, "mapAStar") == 0) {
        grid_cols = numCols;
        grid_rows = numRows;

        // Initialize the grid map with the new size
        init_empty_grid_map(grid_cols, grid_rows, grid_map_cells);

        // Find the start of the map data
        char *map_start = strstr(ptrBufferReception, "\n") + 1;

        // Split the map into individual lines
        char *line = strtok(map_start, "\n");
        int row = 0;

        // Process each line of the map
        while (line != NULL && row < grid_rows) {
            populate_grid(line, row, grid_map_cells);
            row++;
            line = strtok(NULL, "\n");
        }

        if (row == grid_rows) {
            // All rows received, process the map
            print_map(grid_cols, grid_rows, grid_map_cells);
            A_star_algorithm();
        } else {
            // If not all rows are received, print an error message
            writeString(&handlerTerminal, "Error: Incomplete map data\n");
        }
    } else {
        // If the command does not match, print a "Wrong CMD" message
        printf("Wrong CMD \n");
    }
}
