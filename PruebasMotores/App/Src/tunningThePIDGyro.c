...
double Kp_theta = 11.0; //918.5; // Proportional gain for angle
double Ki_theta = 0.1; //.5;    // Integral gain for angle
double Kd_theta = 1.05; //362.5;   // Derivative gain for angle
...
//Movement variables
GPIO_Handler_t pin_PWMConfig;
BasicTimer_Handler_t handlerTimer3;
BasicTimer_Handler_t handlerTimer2;
PWM_Handler_t pwmBlue = { 0 };
bool dirYellowVal = 0;
PWM_Handler_t pwmYellow = { 0 };
bool dirBlueVal = 0;
		...
		if (accelActivate == 1) {
			if (measure) {
				...
				if (measureCount < aux) {
					...
					currentOffset += sumGyroOffset / aux;
					measureCount++;
				} else if (measureCount < aux * 1.5f) {
					gyroZ = getMeasure();
					if (fabs(gyroZ) > 0.3) { // Reduced threshold for finer control
...
void configPeripherals(void) {
	...
	pin_PWMConfig.GPIO_PinConfig_t.GPIO_PinMode = GPIO_MODE_ALTFN;
	...
	pin_PWMConfig.GPIO_PinConfig_t.GPIO_PinSpeed = GPIO_OSPEED_FAST;
...
void parseCommands(char *ptrBufferReception) {
	 ...
	 * y dos float llamados "firstParameter" y "secondParameter"
...
void BasicTimer2_Callback(void) {
	...
	if (accelActivate == 1) {
		...
		thetaG += gyroZ * 0.025f;
		if (move90G) {
			if (thetaG > tethaObjective * 0.99f) {
				stop();
				move90G = 0;
			}
		}
...
void BasicTimer3_Callback(void) {
	...
	if (counter10avg >= 4 && onMove && initialCalibrate && moves) {
...
//	}
	...
	if (theta > M_PI)
		...
		theta += 2 * M_PI;
	double thetaGRad = thetaG * (M_PI / 180.0);
