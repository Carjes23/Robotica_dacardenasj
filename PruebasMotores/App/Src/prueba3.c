...
PWM_Handler_t pwmDer = { 0 }; //Para configurar el PWM en el timer 3 para X
PWM_Handler_t pwmIzquierdo = { 0 }; //Para configurar el PWM en el timer 3 para X
...
uint16_t prescaler = 4000;
uint16_t periodo = 1000;

uint8_t counterDuttyIz = start;
uint8_t counterDuttyDe = start;
uint16_t duttyIz = 1000 * start / 100;
uint16_t duttyDe = 1000 * start / 100;
...
void configPeripherals(void) {
	...
	pwmIzquierdo.ptrTIMx = TIM5;
	pwmIzquierdo.config.channel = PWM_CHANNEL_2;
	pwmIzquierdo.config.duttyCicle = duttyIz;
	pwmIzquierdo.config.periodo = periodo;
	pwmIzquierdo.config.prescaler = prescaler;
	pwmIzquierdo.config.polarity = 0;

	pwm_Config(&pwmIzquierdo);
	enableOutput(&pwmIzquierdo);
	startPwmSignal(&pwmIzquierdo);
//	stopPwmSignal(&pwmIzquierdo);

	pwmDer.ptrTIMx = TIM5;
	pwmDer.config.channel = PWM_CHANNEL_1;
	pwmDer.config.duttyCicle = duttyIz;
	pwmDer.config.periodo = periodo;
	pwmDer.config.prescaler = prescaler;
	pwmDer.config.polarity = 0;

	pwm_Config(&pwmDer);
	enableOutput(&pwmDer);
	startPwmSignal(&pwmDer);
...
void parseCommands(char *ptrBufferReception) {
	...
	if (strcmp(cmd, "help") == 0) {
		...
		pwmUpdatePolarity(&pwmIzquierdo, firstParameter);
	}
...
void BasicTimer3_Callback(void) {
	...
	count10seg++;
	if (count10seg > 4 * 10 - 1) {
		sprintf(bufferData, "%d\t%d\t%d\n", counterDuttyIz, PC1Counter,
				PC3Counter);
		writeString(&handlerTerminal, bufferData);
		count10++;
		if (count10 > 2) {
			if (counterDuttyIz < 27) {
				counterDuttyIz++;

			} else {
				counterDuttyIz = start;

			}
			pwmIzquierdo.config.duttyCicle = duttyIz = periodo * counterDuttyIz
					/ 100;
			setDuttyCycle(&pwmIzquierdo);
			pwmDer.config.duttyCicle = duttyIz = periodo * counterDuttyIz / 100;
			setDuttyCycle(&pwmDer);
			count10 = 0;
		}
