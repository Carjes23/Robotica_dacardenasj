	...
	if (counter10avg >= 8) {
		if (onMove) {
			...
			if (initialCalibrate || calibrate) {
				if (pwmYellow.config.duttyCicle >= 2500
						&& pwmYellow.config.duttyCicle <= 4000) {
					setDuttyCycle(&pwmYellow);
				} else if (pwmYellow.config.duttyCicle < 2500) {
					duttyIYwl = 2500;
				} else {
					duttyIYwl = 4000;
				}
				if (pwmBlue.config.duttyCicle >= 2500
						&& pwmBlue.config.duttyCicle <= 4000) {
					setDuttyCycle(&pwmBlue);
				} else if (pwmBlue.config.duttyCicle < 2500) {
					duttyInBlue = 2500;
				} else {
					duttyInBlue = 4000;
				}
